...
def sweep_abfrage(predefined_sweep=None):
    """Fragt Sweep-Parameter ab oder übernimmt vordefinierte."""
    if predefined_sweep is not None:
        return predefined_sweep
...
def save_to_csv(data, filename, mode_label):
    # Definiert den Unterordner "messungen" im aktuellen Verzeichnis
    ziel_ordner = "./Messergebnisse"
    
    # Erstellt den Ordner automatisch, falls er noch nicht existiert
    if not os.path.exists(ziel_ordner):
        os.makedirs(ziel_ordner)
        print(f"[INFO] Neuer Ordner erstellt: {ziel_ordner}")
        
    # Verbindet den Ordnerpfad mit dem Dateinamen (z.B. ./messungen/langmuir_einfach_...)
    vollstaendiger_pfad = os.path.join(ziel_ordner, filename)
    
    try:
        with open(vollstaendiger_pfad, mode='w', newline='', encoding='utf-8') as f:
            writer = csv.writer(f)
            writer.writerow(["Zeitstempel_PC", "DMM_Zeit_Rel_S", "Spannung_Soll_V", "Spannung_Ist_V", "Strom_Ist_A", "Strom_SMU_A", "Modus"])
            writer.writerows(data)
        print(f"[ERFOLG] Daten erfolgreich gespeichert unter: {vollstaendiger_pfad}")
    except Exception as e:
...
def modus_1_einfache_sonde(mux, smu, alle_kanaele, predefined_sweep=None, predefined_channel=None):
    print("\n=== MODUS 1: EINFACHE LANGMUIRSONDE ===")
    
    if predefined_channel is not None:
        kanal_raw = predefined_channel
    else:
        kanal_raw = input("Welcher Multiplexer-Kanal wird verwendet? (z.B. 1002): ").strip().replace("@", "")
        
    if kanal_raw not in alle_kanaele: 
        print("[FEHLER] Ungültiger Kanal!")
        return
        
    spannungen = sweep_abfrage(predefined_sweep)
    ...
    try:
        ...
        print(f"\nStarte Messreihe an Kanal {kanal_raw} (Software-Sweep)...")
...
def modus_2_guardring_sonde(mux, smu, k_dmm, alle_kanaele, predefined_sweep=None, predefined_guard=None, predefined_plate=None):
    print("\n=== MODUS 2: LANGMUIRSONDE MIT GUARDRING ===")
    
    if predefined_guard is not None and predefined_plate is not None:
        kanal_guard = predefined_guard
        kanal_plate = predefined_plate
    else:
        kanal_guard = input("Kanal für GUARDRING (z.B. 1001): ").strip().replace("@", "")
        kanal_plate = input("Kanal für MESSPLATTE (z.B. 1002): ").strip().replace("@", "")
    
    if (kanal_guard not in alle_kanaele) or (kanal_plate not in alle_kanaele):
    ...
    try:
        ...
        mux.safe_switch_to_guardring(guard_channel=kanal_guard, plate_channel=kanal_plate)
        time.sleep(0.5)
        smu.output_on()
        ...
        print(f"\nStarte präzisen High-Speed Guardring-Sweep (Guard: {kanal_guard}, Platte: {kanal_plate})...")
        for u_soll in spannungen:
            smu.set_voltage(u_soll)
            time.sleep(0.01) 
...
def modus_3_beide_messungen(mux, smu, k_dmm, alle_kanaele):
    """MODUS 3: Fragt alle Kanäle und Parameter vorab ab und misst dann vollautomatisch durch."""
    print("\n=== MODUS 3: BEIDE MODI HINTEREINANDER ===")
    
    # 1. KANALABFRAGEN VORAB
    print("\n--- KANALEINSTELLUNGEN FÜR DIE MESSREIHE ---")
    k1_sonde = input("Modus 1 - Welcher Sonden-Kanal? (z.B. 1002): ").strip().replace("@", "")
    k2_guard = input("Modus 2 - Kanal für GUARDRING?  (z.B. 1001): ").strip().replace("@", "")
    k2_platte = input("Modus 2 - Kanal für MESSPLATTE? (z.B. 1002): ").strip().replace("@", "")
    
    # Validierung
    if (k1_sonde not in alle_kanaele) or (k2_guard not in alle_kanaele) or (k2_platte not in alle_kanaele):
        print("[FEHLER] Mindestens einer der eingegebenen Kanäle ist ungültig!")
        return

    # 2. PARAMETERABFRAGE VORAB
    spannungen = sweep_abfrage()
    
    # 3. AUTOMATISCHE AUSFÜHRUNG
    try:
        print("\n--- SCHRITT 1/2: Starte einfachen Sonden-Sweep ---")
        modus_1_einfache_sonde(mux, smu, alle_kanaele, predefined_sweep=spannungen, predefined_channel=k1_sonde)
        
        print("\nSchnittstellen-Pause vor Modus 2...")
        time.sleep(1.5)
        
        print("\n--- SCHRITT 2/2: Starte Guardring Sonden-Sweep ---")
        modus_2_guardring_sonde(mux, smu, k_dmm, alle_kanaele, predefined_sweep=spannungen, predefined_guard=k2_guard, predefined_plate=k2_platte)
        
    except Exception as e:
        print(f"\n[FEHLER] Abbruch in Kombi-Modus 3: {e}")
    finally:
        try:
            smu.output_off()
            mux.open_all()
...
if __name__ == "__main__":
    ...
    try:
        ...
        while True:
            ...
            print("[3] Beide Modis hintereinander (Vorauswahl)")
            print("[4] Programm beenden")
            
            auswahl = input("Bitte Modus wählen (1-4): ").strip()
            if auswahl == "1":
                modus_1_einfache_sonde(mux, smu, SONDEN_KANAELE)
            elif auswahl == "2":
                modus_2_guardring_sonde(mux, smu, k_dmm, SONDEN_KANAELE)
            elif auswahl == "3":
                modus_3_beide_messungen(mux, smu, k_dmm, SONDEN_KANAELE)
            elif auswahl == "4":
