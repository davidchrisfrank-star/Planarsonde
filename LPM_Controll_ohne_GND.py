...
class KeysightMultiplexer:
    """Steuerung des Keysight 34923A/34980A Multiplexers mit BBM-Sicherheit."""
    ...
    def safe_switch_to_channel(self, active_channel):
        """
...
def modus_1_einfache_sonde(mux, smu, alle_kanaele):
    ...
    try:
        ...
        # 2. Sicherer Schaltvorgang mittels BBM (Inaktive Kanäle bleiben unberührt/offen)
        mux.safe_switch_to_channel(active_channel=kanal_raw)
        time.sleep(0.5)
        mux.send("ROUTe:CLOS (@1911)")
        time.sleep(0.5)
...
def modus_2_guardring_sonde(mux, smu, k_dmm, alle_kanaele):
    """Führt einen Sweep mit Guardring durch (Beide aktiv via BBM, inaktive Kanäle ungeschaltet)."""
    ...
    try:
        ...
        if mux.simulate:
            ...
            # BREAK: Alle Analogbusse (1-4) und Kanäle komplett öffnen
            mux.instrument.write("ROUTe:OPEN (@1911,1912,1913,1914)")
            mux.instrument.write("ROUT:OPEN:ALL")
            mux.instrument.query("*OPC?")
            
            # MAKE: Guard an ABus1 (SMU) und Platte an ABus2 (DMM-Pfad) legen. 
            # Inaktive Kanäle werden hier nicht mehr auf ABus3 gelegt und bleiben floatend/offen.
            mux.instrument.write(f"ROUTe:CLOS (@1911)")
            mux.instrument.write(f"ROUT:CLOS (@{kanal_guard})")  
            mux.instrument.write(f"ROUTe:CLOS (@1912)") 
            mux.instrument.write(f"ROUT:CLOS (@{kanal_platte})") 
            mux.instrument.query("*OPC?")
