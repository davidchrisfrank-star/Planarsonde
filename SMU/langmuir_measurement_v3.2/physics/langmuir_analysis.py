...
Analysis pipeline (in order):
  1. Smoothing (Savitzky-Golay / Gaussian / Moving-Average / Median)
...
Filter methods available:
  "savgol"   – Savitzky-Golay (default, preserves peak shape best)
  "gaussian" – Gaussian convolution (sigma in points, set via savgol_window)
  "moving"   – Simple moving-average (window = savgol_window)
  "median"   – Median filter (window = savgol_window, robust against spikes)
...
FILTER_METHODS = ("savgol", "gaussian", "moving", "median", "none")
...
class LangmuirAnalyzer:
    ...
    savgol_window : int
        Filter window length (used by all filter methods).
    ...
    filter_method : str
        One of "savgol" (default), "gaussian", "moving", "median", "none".
    ...
    def __init__(
        ...
        if filter_method not in FILTER_METHODS:
            raise ValueError(
                f"filter_method must be one of {FILTER_METHODS}, got '{filter_method}'"
            )
    ...
    def _smooth(self) -> np.ndarray:
        """
        Step 1 — Apply the selected filter.

        filter_method options:
          savgol   – Savitzky-Golay (polynomial fit, preserves peak shape)
          gaussian – Gaussian convolution (sigma ≈ window/6)
          moving   – Uniform moving-average (box kernel)
          median   – Median filter (robust against isolated spikes)
          none     – Raw data passed through unchanged
        """
        n = len(self.I)
        ...
        w = max(w, self.savgol_polyorder + 2)
        if w % 2 == 0:
            w += 1
        ...
        if self.filter_method == "gaussian":
            # sigma chosen so that 3σ ≈ half-window  →  σ = w / 6
            sigma = max(w / 6.0, 1.0)
            return gaussian_filter1d(self.I, sigma=sigma)

        if self.filter_method == "moving":
            kernel = np.ones(w) / w
            # 'same' mode + edge padding mirrors scipy behaviour
            return np.convolve(self.I, kernel, mode="same")

        if self.filter_method == "median":
            return medfilt(self.I, kernel_size=w)

        # Fallback (should never reach here due to __init__ guard)
        return savgol_filter(self.I, w, self.savgol_polyorder)
