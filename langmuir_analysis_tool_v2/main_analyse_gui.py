...
from physics.langmuir_analysis import (
    ...
    berechne_plasmadichte, IONENMASSEN, PlasmadichteErgebnis,
)
...
class Datensatz:
    ...
    def __init__(
        ...
        self.flaeche_m2: Optional[float]                = None
        self.plasma_ergebnis: Optional[PlasmadichteErgebnis] = None
...
class AnalyseGUI(tk.Tk):
    ...
    # ── Tab 4: Plasma ────────────────────────────────────────────────────
    def _tab_plasma(self, nb: ttk.Notebook) -> None:
        tab = ttk.Frame(nb)
        nb.add(tab, text="Plasma")

        ttk.Label(tab, text="Plasmadichte  nₑ  (Bohm-Kriterium)",
                  font=("", 10, "bold")).pack(anchor="w", padx=8, pady=(8, 4))

        ttk.Separator(tab, orient=tk.HORIZONTAL).pack(fill=tk.X, padx=8, pady=4)

        # ── Sondenoberfläche ──────────────────────────────────────────────
        ttk.Label(tab, text="Sondenoberfläche (mm²):",
                  font=("", 9)).pack(anchor="w", padx=8)
        f_fl = ttk.Frame(tab)
        f_fl.pack(fill=tk.X, padx=8, pady=(2, 6))
        self._plasma_flaeche_var = tk.StringVar(value="")
        fl_entry = ttk.Entry(f_fl, textvariable=self._plasma_flaeche_var, width=10)
        fl_entry.pack(side=tk.LEFT)
        _tooltip(fl_entry, "Oberfläche der aktiven Sonde in mm².\nDezimaltrennzeichen: Punkt oder Komma.")
        ttk.Button(f_fl, text="← aus Vergleich-Tab",
                   command=self._plasma_flaeche_aus_vergleich).pack(
            side=tk.LEFT, padx=(6, 0))
        _tooltip(f_fl.winfo_children()[-1],
                 "Übernimmt den ersten Wert aus dem Vergleich-Tab.")

        # ── Gasauswahl ────────────────────────────────────────────────────
        ttk.Label(tab, text="Gas / Ionensorte:",
                  font=("", 9)).pack(anchor="w", padx=8)
        self._plasma_gas_var = tk.StringVar(value="Argon")
        gas_werte = list(IONENMASSEN.keys()) + ["Manuell"]
        gas_cb = ttk.Combobox(tab, textvariable=self._plasma_gas_var,
                              values=gas_werte, state="readonly", width=18)
        gas_cb.pack(anchor="w", padx=8, pady=(2, 4))
        gas_cb.bind("<<ComboboxSelected>>", self._plasma_gas_gewaehlt)

        # Manuelle Ionenmasse — zunächst versteckt
        self._plasma_manuell_frame = ttk.Frame(tab)
        self._plasma_manuell_frame.pack(fill=tk.X, padx=8, pady=(0, 4))
        ttk.Label(self._plasma_manuell_frame,
                  text="Ionenmasse (amu):").pack(side=tk.LEFT)
        self._plasma_masse_var = tk.StringVar(value="")
        ttk.Entry(self._plasma_manuell_frame,
                  textvariable=self._plasma_masse_var,
                  width=8).pack(side=tk.LEFT, padx=(6, 0))
        self._plasma_manuell_frame.pack_forget()   # initial verborgen

        ttk.Separator(tab, orient=tk.HORIZONTAL).pack(fill=tk.X, padx=8, pady=6)

        # ── Berechnen-Button ──────────────────────────────────────────────
        ttk.Button(tab, text="▶  Plasmadichte berechnen",
                   command=self._plasmadichte_berechnen).pack(
            fill=tk.X, padx=8, pady=(0, 6))

        # ── Ergebnis-Anzeige ─────────────────────────────────────────────
        ttk.Label(tab, text="Ergebnis:", font=("", 9, "bold")).pack(
            anchor="w", padx=8)
        self._plasma_ergebnis_var = tk.StringVar(value="Noch nicht berechnet.")
        ttk.Label(tab, textvariable=self._plasma_ergebnis_var,
                  font=("Courier New", 8), foreground="#003366",
                  justify=tk.LEFT, wraplength=285).pack(
            anchor="w", padx=8, pady=(2, 6))

        # Hinweis / Fehlermeldung
        self._plasma_hinweis_var = tk.StringVar(value="")
        ttk.Label(tab, textvariable=self._plasma_hinweis_var,
                  foreground="#cc4400", font=("", 8),
                  justify=tk.LEFT, wraplength=285).pack(
            anchor="w", padx=8)
    ...
    def _datei_auswaehlen(self, _event=None) -> None:
        ...
        # Plasma-Tab Anzeige synchronisieren
        if ds.plasma_ergebnis is not None:
            self._plasma_ergebnis_var.set(ds.plasma_ergebnis.zusammenfassung())
            if ds.flaeche_m2 is not None:
                self._plasma_flaeche_var.set(
                    f"{ds.flaeche_m2 * 1e6:.4g}".replace(".", ","))
        else:
            self._plasma_ergebnis_var.set("Noch nicht berechnet.")
    ...
    def _datei_entfernen(self) -> None:
        ...
        self._meta_var.set("—")
        self._plasma_ergebnis_var.set("Noch nicht berechnet.")
        self._plasma_hinweis_var.set("")

    # ======================================================================= #
    #  Plasma-Ereignisbehandlung
    # ======================================================================= #

    def _plasma_gas_gewaehlt(self, _event=None) -> None:
        """Zeigt/versteckt das Manuell-Eingabefeld je nach Gasauswahl."""
        if self._plasma_gas_var.get() == "Manuell":
            self._plasma_manuell_frame.pack(fill=tk.X, padx=8, pady=(0, 4))
        else:
            self._plasma_manuell_frame.pack_forget()

    def _plasma_flaeche_aus_vergleich(self) -> None:
        """Übernimmt den ersten Flächenwert aus dem Vergleich-Tab."""
        rohwert = self._flaechen_var.get().strip()
        if not rohwert:
            self._plasma_hinweis_var.set(
                "Kein Wert im Vergleich-Tab eingetragen.")
            return
        erster = rohwert.split(";")[0].strip()
        self._plasma_flaeche_var.set(erster)
        self._plasma_hinweis_var.set("")

    def _plasmadichte_berechnen(self) -> None:
        """Liest Parameter, berechnet n_e und aktualisiert Anzeige + Plot."""
        # Aktiven Datensatz ermitteln
        if self._aktiver_idx < 0 or self._aktiver_idx >= len(self._datensaetze):
            self._plasma_hinweis_var.set(
                "Bitte zuerst eine Datei aus der Liste auswählen.")
            return
        ds = self._datensaetze[self._aktiver_idx]
        if ds.results is None:
            self._plasma_hinweis_var.set(
                "Datei noch nicht analysiert — bitte zuerst Analyse starten.")
            return

        # Sondenoberfläche lesen
        try:
            A_mm2 = float(
                self._plasma_flaeche_var.get().strip().replace(",", "."))
        except ValueError:
            self._plasma_hinweis_var.set(
                "Ungültige Oberfläche. Zahl in mm² eingeben (z. B. 12,5).")
            return
        if A_mm2 <= 0:
            self._plasma_hinweis_var.set(
                "Oberfläche muss > 0 mm² sein.")
            return

        # Gas / Ionenmasse lesen
        gas      = self._plasma_gas_var.get()
        m_i_amu: float | None = None
        if gas == "Manuell":
            try:
                m_i_amu = float(
                    self._plasma_masse_var.get().strip().replace(",", "."))
            except ValueError:
                self._plasma_hinweis_var.set(
                    "Ungültige Ionenmasse. Zahl in amu eingeben.")
                return
            if m_i_amu <= 0:
                self._plasma_hinweis_var.set("Ionenmasse muss > 0 amu sein.")
                return

        # Berechnung
        try:
            ergebnis = berechne_plasmadichte(
                ds.results, A_mm2=A_mm2,
                gas=gas, m_i_amu=m_i_amu,
            )
        except Exception as e:
            self._plasma_hinweis_var.set(f"Berechnungsfehler: {e}")
            return

        # Ergebnis speichern und anzeigen
        ds.plasma_ergebnis = ergebnis
        ds.flaeche_m2      = A_mm2 * 1e-6

        self._plasma_ergebnis_var.set(ergebnis.zusammenfassung())
        self._plasma_hinweis_var.set("")
        self._log(f"\n[Plasma] {ds.pfad.name}\n"
                  f"{ergebnis.zusammenfassung()}\n", _TAG_OK)
        self._status(f"Plasmadichte berechnet — {ds.pfad.name}")

        # Plot mit Panel 4 neu zeichnen
        self._plot_aktualisieren(ds)
    ...
    def _plot_aktualisieren(self, ds: Datensatz) -> None:
        ...
        plotter = LangmuirPlotter(ds.V, ds.I, ds.analyzer, ds.results,
                                  label=ds.label,
                                  plasma_ergebnis=ds.plasma_ergebnis)
        self._fig = plotter.build(figgroesse=(8, 14))
