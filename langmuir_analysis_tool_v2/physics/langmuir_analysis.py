...
class LangmuirAnalyzer:
    ...
    # Bei Keithley-DMM-Messungen (µA-Bereich) ist das praktische Rauschen deutlich höher
    # (~1–10 nA). Falls der T_e-Fit zu wenige Punkte liefert, kann ein benutzerdefinierter
    # Wert über den LangmuirAnalyzer-Konstruktor übergeben werden (kein separater Parameter
    # nötig — Subklasse mit überschriebenem RAUSCHGRENZE-Attribut).
    RAUSCHGRENZE: float = 10e-15  # A = 10 fA (theoretische SMU-Grenze)
    ...
    def _glaetten(self) -> np.ndarray:
        ...
        if dV == 0.0:
            raise AnalysisError(
                f"Alle Spannungswerte sind identisch (V = {self.V[0]:.4g} V). "
                "Keine Analyse möglich — bitte Sweep-Daten prüfen."
            )
    ...
    def _schwebepotential_finden(self, I_s: np.ndarray) -> float:
        ...
        dI = I_s[idx + 1] - I_s[idx]
        dV = self.V[idx + 1] - self.V[idx]
        if abs(dI) < 1e-15:
            V_fl = float(self.V[idx])   # Näherung wenn Strom nahezu konstant
        else:
            V_fl = float(self.V[idx] - I_s[idx] * (dV / dI))
        return V_fl
    ...
    def _plasmapotential_finden(self, dIdV: np.ndarray, V_fl: float) -> float:
        ...
        if maske.sum() == 0:
            raise AnalysisError(
                "Keine Spannungspunkte ≥ V_fl für die V_p-Suche verfügbar. "
                "Sweep-Bereich endet möglicherweise vor dem Schwebepotential."
            )
    ...
    def _ionensaettigung_fitten(
        ...
        # Warnung wenn Fit-Bereich sehr klein ist (typisch bei engem Sweep)
        if (v_max - v_min) < 5.0:
            warnings.warn(
                f"Ionensättigungs-Fit-Bereich sehr klein: {v_max - v_min:.1f} V "
                f"(von {v_min:.1f} bis {v_max:.1f} V). "
                "Sweep-Bereich ggf. zu eng oder ion_fit_range anpassen. "
                "Ionensättigungsstrom könnte ungenau sein.",
                stacklevel=3,
            )
...
# ====================================================================== #
#  Plasmadichte nach Bohm-Kriterium
# ====================================================================== #

_U_KG = 1.66054e-27   # atomare Masseneinheit in kg
_E    = 1.60218e-19   # Elementarladung in C

# Ionenmassen typischer Arbeitsgase in kg
IONENMASSEN: dict[str, float] = {
    "Argon":        39.948 * _U_KG,
    "Helium":        4.003 * _U_KG,
    "Neon":         20.180 * _U_KG,
    "Stickstoff":   28.014 * _U_KG,   # N₂ (Molekül)
    "Wasserstoff":   2.016 * _U_KG,   # H₂ (Molekül)
    "Sauerstoff":   32.000 * _U_KG,   # O₂ (Molekül)
    "Krypton":      83.798 * _U_KG,
    "Xenon":       131.293 * _U_KG,
}


@dataclass
class PlasmadichteErgebnis:
    """Ergebnis der Plasmadichte-Berechnung nach Bohm-Kriterium."""
    n_e_m3:    float   # Plasmadichte in m⁻³
    n_e_cm3:   float   # Plasmadichte in cm⁻³
    v_bohm:    float   # Bohm-Geschwindigkeit in m/s
    I_ion_sat: float   # verwendeter Ionensättigungsstrom (A, negativ)
    T_e_eV:    float   # verwendete Elektronentemperatur (eV)
    A_m2:      float   # verwendete Sondenoberfläche (m²)
    m_i_kg:    float   # verwendete Ionenmasse (kg)
    gas:       str     # Gasbezeichnung

    def zusammenfassung(self) -> str:
        return (
            f"n_e  = {self.n_e_m3:.4e} m⁻³\n"
            f"n_e  = {self.n_e_cm3:.4e} cm⁻³\n"
            f"v_Bohm = {self.v_bohm:.0f} m/s\n"
            f"Gas:   {self.gas}\n"
            f"T_e:   {self.T_e_eV:.2f} eV\n"
            f"I_ion: {self.I_ion_sat * 1e3:.4g} mA\n"
            f"A:     {self.A_m2 * 1e6:.4g} mm²"
        )


def berechne_plasmadichte(
    results:   "LangmuirResults",
    A_mm2:     float,
    gas:       str   = "Argon",
    m_i_amu:   float | None = None,
) -> PlasmadichteErgebnis:
    """
    Plasmadichte nach Bohm-Kriterium berechnen.

    n_e = |I_ion_sat| / (0.61 · e · A · v_Bohm)
    v_Bohm = sqrt(e · T_e_eV / m_i)

    Parameter
    ----------
    results  : LangmuirResults  Analyseergebnis (braucht T_e und I_ion_sat)
    A_mm2    : float             Sondenoberfläche in mm²
    gas      : str               Gasbezeichnung (Schlüssel in IONENMASSEN)
    m_i_amu  : float | None      Manuelle Ionenmasse in amu (überschreibt gas)
    ...
    """
    if A_mm2 <= 0:
        raise ValueError(f"Sondenoberfläche muss > 0 mm² sein, erhalten: {A_mm2}")
    if results.T_e <= 0:
        raise ValueError(f"Elektronentemperatur muss > 0 eV sein, erhalten: {results.T_e}")

    A_m2 = A_mm2 * 1e-6

    if m_i_amu is not None:
        m_i = m_i_amu * _U_KG
        gas_name = f"Manuell ({m_i_amu:.3g} amu)"
    else:
        if gas not in IONENMASSEN:
            raise ValueError(
                f"Unbekanntes Gas '{gas}'. Verfügbar: {list(IONENMASSEN.keys())}"
            )
        m_i = IONENMASSEN[gas]
        gas_name = gas

    v_bohm = np.sqrt(_E * results.T_e / m_i)
    n_e    = abs(results.I_ion_sat) / (0.61 * _E * A_m2 * v_bohm)

    return PlasmadichteErgebnis(
        n_e_m3    = float(n_e),
        n_e_cm3   = float(n_e * 1e-6),
        v_bohm    = float(v_bohm),
        I_ion_sat = results.I_ion_sat,
        T_e_eV    = results.T_e,
        A_m2      = A_m2,
        m_i_kg    = m_i,
        gas       = gas_name,
