...
from physics.langmuir_analysis import LangmuirAnalyzer, LangmuirResults, PlasmadichteErgebnis
...
class LangmuirPlotter:
    ...
    def __init__(
        ...
        label: str = "",
        plasma_ergebnis: Optional[PlasmadichteErgebnis] = None,
    ) -> None:
        self.V      = V
        self.I      = I
        self.az     = analyzer
        self.res    = results
        self.label  = label
        self.plasma = plasma_ergebnis

    # ------------------------------------------------------------------ #

    def build(self, figgroesse: tuple = (11, 18)) -> plt.Figure:
        """Vierteilige Abbildung aufbauen und zurückgeben."""
        ...
        self._panel_plasmadichte(ax_pd)
    ...
    # ------------------------------------------------------------------ #
    #  Panel 4 – Plasmadichte
    # ------------------------------------------------------------------ #

    def _panel_plasmadichte(self, ax: plt.Axes) -> None:
        """Panel 4: Plasmadichte-Ergebnis als Info-Tafel."""
        ...
        ax.set_title("Panel 4 — Plasmadichte $n_e$  (Bohm-Kriterium)",
                     fontsize=11, fontweight="bold")

        if self.plasma is None:
            ax.text(0.5, 0.5,
                    "Plasmadichte nicht berechnet.\n"
                    "Sondenoberfläche und Gas im Tab »Plasma« eingeben,\n"
                    "dann »Plasmadichte berechnen« drücken.",
        ...
        links = (
            ...
            "─────────────────────────────\n"
            f"$n_e$  =  {p.n_e_m3:.4e}  m⁻³\n"
            f"$n_e$  =  {p.n_e_cm3:.4e}  cm⁻³\n\n"
            f"$v_{{Bohm}}$  =  {p.v_bohm:.0f}  m/s"
        )

        # ── rechte Spalte: verwendete Parameter ───────────────────────────
        rechts = (
            "Verwendete Parameter\n"
            "─────────────────────────────\n"
            f"Gas:       {p.gas}\n"
            f"$T_e$:       {p.T_e_eV:.2f} eV\n"
            f"$I_{{ion,sat}}$:  {p.I_ion_sat * 1e3:.4g} mA\n"
            f"$A_{{Sonde}}$:  {p.A_m2 * 1e6:.4g} mm²\n"
            f"$m_i$:      {p.m_i_kg / 1.66054e-27:.3g} amu"
        )

        # Trennlinie vertikal
        ax.axvline(x=0.5, ymin=0.05, ymax=0.95,
                   color="#cccccc", lw=1.0, transform=ax.transAxes)

        ax.text(0.03, 0.95, links,
                transform=ax.transAxes, fontsize=9.5, va="top",
                fontfamily="monospace", color="#003366",
                linespacing=1.6)
        ax.text(0.53, 0.95, rechts,
                transform=ax.transAxes, fontsize=9.5, va="top",
